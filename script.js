/* ─────────────────────────────────────────
   Ever After Planner — script.js v2
   Includes: progress wheels, palette preview, RSVP ring
───────────────────────────────────────── */
...
// ─ Ring constants (r=85, so circumference = 2 * PI * 85)
...
function drawDonut() {
  ...
  if (entries.length === 0) {
    ...
    ctx.fill();
    ctx.fillStyle = '#96a398';
    ctx.font = '13px DM Sans';
    ctx.textAlign = 'center';
    ctx.fillText('No expenses yet', cx, cy + 5);
...
function catColor(cat) {
  const map = { Venue:'#8fac82',Catering:'#f0b89a',Photography:'#d4e8d0',Flowers:'#f7d8c5',Music:'#c5d4b0',Attire:'#faf3e8',Cake:'#a3b898',Transport:'#fcefe6',Honeymoon:'#7a9970',Stationery:'#d4b8a8',Other:'#c09884' };
